...
class UIXMLTree:
    ...
    def process(self, xml_string, app_name, level=1, str_type="json", remove_system_bar=True, use_bounds=False,
                merge_switch=False):
    ...
    def dict_to_plain_text(self, xml_dict, indent=0):
        result = ""
        filter_value = ['view_hash', 'page_fragment_hash', 'input-type', 'string-res', 'hint', 'important-for-a11y', 'tag-page_code_fragment', 'span-node',
                        'tag-page_code_activity', 'tag-page_xml', 'tag', 'alpha', 'tag-page_name_activity', 'base-class', 'bounds',
                        'tag-version_name', 'tag-git_branch', 'tweak_element_id', 'visible', 'activated', 'has-window-focus', 'scroll-type', 'scroll-direction']
        for key, value in xml_dict.items():
            if isinstance(value, dict):
                if key not in filter_value:
                    str_key = str(key)
                    index = str_key.find(']')
                    if index != -1:
                        str_key = str_key[index + 2:]
                    result += " " * indent + str_key
                result += "\n" + self.dict_to_plain_text(value, indent + 4)
            elif key == 'tweak_element_id' and value != '':
                result += " " * indent + "element description: "
                result += str(value).rsplit('/', 1)[-1] + "\n"
            elif key == 'tag-page_name_fragment' and value != '':
                result += " " * indent + "fragment: "
                result += str(value).rsplit('.', 1)[-1] + "\n"
            elif str(value) != '' and key not in filter_value and str(value) != 'none':
    ...
    def merge_attribute_in_one_line(self, node):
        ...
        if node.attrib['action'] != "":
            node.attrib['description'] += ';' + node.attrib.get('action', '') + '; '

        # status
        for attrib in ['checked', 'password', 'selected']:
            if attrib in node.attrib and node.attrib[attrib] == "true":
        ...
        if node.attrib['func-desc'] != '':
            node.attrib['description'] += "; " + node.attrib.get('func-desc', '')
        node.attrib['description'] = node.attrib['description'].replace("\n", "")
        # map functional attributes to support actions

        if node.attrib.get("tweak_element_id", "") != '':
            node.attrib['description'] += "; " + node.attrib['tweak_element_id'].rsplit('/', 1)[-1]
        # if node.attrib.get('editable', '') != '':
        #     node.attrib['description'] += "; " + 'editable:' + node.attrib['editable']

        # clean attribute
        for attrib in ['index', 'text', 'resource-id', 'package', 'content-desc', 'enabled', 'focused',
                       'visible', 'bounds', 'class', 'checkable', 'checked', 'clickable', 'focusable',
                       'scrollable', 'long-clickable', 'password', 'tweak_element_id', 'editable',
    ...
    def get_xpath(self, node):
        if node.tag == 'hierarchy':
            return '/'
        elif  node.tag == 'span-node':
            parent = node.getparent()
            if parent is not None:
                return parent.attrib['xpath2'] + '/' + parent.attrib['class']
            else:
                return '/'
        else:
            ...
            parent = node.getparent()
            if parent is not None:
                children = parent.xpath(f'./*[@class="{node.attrib.get("class", "")}"]')
    ...
    def get_xpath_new(self, node):
        # 为给定的 XML 节点生成一个唯一的 XPath 表达式，来实现能否将子节点往上合并的过程
        array = []
        while node is not None:
            ...
            if self.get_attr_count("tag", node.tag) == 1:
                ...
                break
            elif self.get_attr_count("text", node.attrib.get("text", "")) == 1:
                array.append(f'*[@text="{node.attrib["text"]}"]')
                break
            elif self.get_attr_count("content-desc", node.attrib.get("content-desc", "")) == 1:
    ...
    def mid_order_remove(self, node):
        # 决定是否移除节点、重组节点属性和处理子节点
        children = list(node)
        node.attrib['name'] = ""
        if node.tag == 'node' or node.tag == 'lynx-node':
            if self.should_remove_node(node) and node.getparent() is not None:
                # remove node
                parent = node.getparent()
                ...
                if node.attrib['class'] != "":
                    ...
                    elif 'content-desc' in node.attrib and node.attrib['content-desc'] != "":
                ...
                # pre action
                for k, v in {
                    'clickable': 'click', 'scrollable': 'scroll', 'long-clickable': 'long-click',
                    'checkable': 'check'}.items():
                    if k in node.attrib and node.attrib[k] == "true":
                ...
                parent = node.getparent()
                if parent is not None and parent.tag == 'node' and "material_clock_face" in parent.attrib['resource-id']:
    ...
    def mid_order_reindex(self, node):
        ...
        self.node_to_name[node.tag] = node.attrib.get("name", "")
    ...
    def xml_sparse(self):
        ...
        for element in self.root.iter():
            ...
            if element.tag != "node":
                continue
            # 使用get方法安全访问属性
            self.inc_attr_count("resource-id", element.attrib.get("resource-id", ""))
            # if "text" in element.attrib:
            self.inc_attr_count("text", element.attrib.get("text", ""))
            self.inc_attr_count("class", element.attrib.get("class", ""))
            # if "content-desc" in element.attrib:
            self.inc_attr_count("content-desc", element.attrib.get("content-desc", ""))
            self.inc_attr_count("tag-page_xml", element.attrib.get("tag-page_xml", ""))
