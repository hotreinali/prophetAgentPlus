...
class Uiautomator2_Helper:
    ...
    def __view_tree_to_list(self, view_tree, view_list, view_shrink_view, depth):
        ...
        if 7 <= depth <= 12:
            view_shrink_view.append(view_tree)
        ...
        for child_tree in view_tree['children']:
            ...
            self.__view_tree_to_list(child_tree, view_list, view_shrink_view, depth + 1)
    ...
    def select_target_root_node(self, xml):
        ...
        for child in root:
            if child.tag == "node" and child.get("package") in exlude_package and child.get("resource_id") != "com.zhiliaoapp.musically:id/float_view":
                continue
            return child
    ...
    def dump_view(self) -> Dict:
        ...
        """
        # 获取uiautomator dump出来的xml file
        xml = self.u2.dump_hierarchy()
        # 因为uiautomator dump出来的xml file中有可能存在多个root node,我们需要选择我们关心的root node
        view_tree = self.select_target_root_node(xml)
        # 解析xml file，返回一个字典。目的是为了和droidbot app中的view tree保持一致
        view_tree = self.xml_to_dict(view_tree)

        return view_tree

    def get_views(self) -> [List, List]:
        """
        ...
        try:
            ...
            if not view_tree:
                print("u2 get views error")
                return None, None
            view_tree['parent'] = -1
            view_list = []
            view_shrink_list = []
            self.__view_tree_to_list(view_tree, view_list, view_shrink_list, 0)
        except Exception as e:
            print(e)
            return None, None
        return view_list, view_shrink_list


    def get_plain_xml(self):
        xml = self.u2.dump_hierarchy()
        # return xml
        exclude_package = ["com.android.systemui","com.github.uiautomator"]
        # iterate all the root nodes from the xml node, and select the one we want
        root = ET.fromstring(xml)
        for child in root:
            if child.tag == "node" and child.get("package") in exclude_package:
                continue
