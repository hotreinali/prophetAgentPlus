...
class Device(object):
    ...
    def __init__(self, device_serial=None, is_emulator=False, output_dir=None,
        ...
        # initialize Uiautomator2_Helper
    ...
    def get_and_read_xml_tree(self):
        # 定义文件路径
        output_file_path = "window_dump.xml"

        # 使用 adb shell 命令来 dump 当前 UI 树并保存到 /sdcard/window_dump.xml
        dump_command = ["adb", "shell", "uiautomator", "dump", "/sdcard/window_dump.xml"]
        subprocess.run(dump_command, check=True)

        # 使用 adb pull 命令将 XML 文件从设备拉取到本地计算机
        pull_command = ["adb", "pull", "/sdcard/window_dump.xml", output_file_path]
    ...
    def get_current_state(self):
        ...
        current_state = None
        try:
            views, views_reduced = self.get_views()
            ...
            try:
                plain_xml = self.uiautomator_helper.get_plain_xml()
                # plain_xml = ""
                # print(type(plain_xml))
    ...
    def get_views(self):
        ...
        if self.uiautomator_helper:
            views, views_reduced = self.uiautomator_helper.get_views()
            if views:
                return views, views_reduced
            else:
