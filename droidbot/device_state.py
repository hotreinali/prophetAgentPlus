...
class DeviceState(object):
    ...
    def __init__(self, device, views, foreground_activity, activity_stack, background_services,
                 tag=None, screenshot_path=None, plain_xml="", views_reduced=None):
        self.device = device
        ...
        self.views_reduced = self.__parse_views(views_reduced)
        self.view_tree = {}
    ...
    def __get_state_str(self):
        ...
        md5_hash = md5(state_str_raw)
        # 创建日志条目
        log_entry = {
            'state_str_raw': state_str_raw,
            'md5_hash': md5_hash
        }
    ...
    def __get_state_str_raw(self):
        if self.device.humanoid is not None:
            ...
            view_signatures = set()
            for view in self.views_reduced:
                # print("each view", view)
                view_signature = DeviceState.__get_content_free_view_signature(view)
                # view_signature = DeviceState.__get_view_signature(view, "rough")
                # print("each view signature", view_signature)
                if view_signature:
    ...
    @staticmethod
    def __get_view_signature(view_dict, mode="precise"):
        """
        ...
        if view_text is None or len(view_text) > 50:
            view_text = "None"
        if mode == "rough":
            signature = "[class]%s[resource_id]%s" % \
